...
class RechargeV extends Model
{
    ...
    public static function exportRechargeRecords($time_begin, $time_end, $type, $admin_id, $username, $company_id)
    {
        $time_end = addDay(1, $time_end);
        $orderings = self::where('company_id', $company_id)
            ...
            ->where('state', CommonEnum::STATE_IS_OK)
            ->field('create_time,username,money,account,type,admin,remark')
            ->order('create_time desc')
            ->select()->toArray();
        return $orderings;
    }
