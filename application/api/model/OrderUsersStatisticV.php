...
class OrderUsersStatisticV extends Model
{
    ...
    public static function getBuildSql3($canteen_id, $consumption_time, $department_id)
    {
        $subQuery = Db::table('canteen_order_t')
            ->alias('a')
            ->field("	`a`.`id` AS `id`,
    ...
    public static function orderStatistic($canteen_id, $consumption_time, $key, $department_id)
    {
        $sql = self::getBuildSql3($canteen_id, $consumption_time, $department_id);
        $statistic = Db::table($sql . ' a')
            ...
            ->field('dinner_id,used,booking,sum(count) as count')
