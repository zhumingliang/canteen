...
class OrderStatisticV extends Model
{
                                  ...
                                  $dinner_id, $type)
    {
        $list = self::whereBetweenTime('ordering_date', $time_begin, $time_end)
            ...
            ->field('order_id,consumption_type,ordering_date,username,canteen,department,phone,count,dinner,type,ordering_type,order_money,1 as status,state,meal_time_end,used')
            ->order('order_id DESC')
            ->paginate($size, false, ['page' => $page]);
        return $list;
