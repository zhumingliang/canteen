...
class UserBalanceV extends Model
{
    ...
    public static function getSqlForStaffsBalance2($department_id, $user, $phone, $companyId)
    {
        $sql = Db::table('canteen_company_staff_t')
            ->alias('b')
            ->field('0 as money,b.id as staff_id,b.username,b.code,c.card_code as card_num,b.phone,b.d_id as department_id,d.name as department,b.state as staff_state')
            ...
            ->where(function ($query) use ($user) {
                if (!empty($user)) {
                    $query->where('b.username|b.code|c.card_num', 'like', '%' . $user . '%');
                }
            })
            ->unionAll(function ($query) use ($companyId, $user, $phone, $department_id) {
                $query->table('canteen_order_t')
                    ->alias('a')
                    ->field('sum(0-a.money-a.sub_money-a.delivery_fee) as money,a.staff_id,b.username,b.code,c.card_code as card_num,b.phone,b.d_id as department_id,d.name as department,b.state as staff_state')
                    ->leftJoin('canteen_company_staff_t b', "a.staff_id=b.id")
                    ->leftJoin('canteen_staff_card_t c', "b.id=c.staff_id and c.state<3")
                    ->leftJoin('canteen_company_department_t d', "b.d_id=d.id")
                    ->where('a.company_id', $companyId)
                    ...
                    ->where(function ($query2) use ($phone) {
                        if (!empty($phone)) {
                            $query2->where('b.phone', $phone);
                        }
                    })
                    ->where(function ($query2) use ($user) {
                        if (!empty($user)) {
                            $query2->where('b.username|b.code|c.card_num', 'like', '%' . $user . '%');
                        }
                    })
                    ->where('a.state', CommonEnum::STATE_IS_OK)
                    ->where('a.pay', PayEnum::PAY_SUCCESS)
                    ->where('b.state', CommonEnum::STATE_IS_OK)->group('a.staff_id');
            })
            ->unionAll(function ($query) use ($companyId, $user, $phone, $department_id) {
                $query->table("canteen_order_parent_t")
                    ->alias('a')
                    ...
                    ->where('a.state', CommonEnum::STATE_IS_OK)
                    ->where('a.pay', PayEnum::PAY_SUCCESS)->group('a.staff_id');
            })
            ->unionAll(function ($query) use ($companyId, $user, $phone, $department_id) {
                $query->table("canteen_shop_order_t")
                    ->alias('a')
                    ->field('sum(0-a.money) as money,a.staff_id,b.username,b.code,c.card_code as card_num,b.phone,b.d_id as department_id,d.name as department,b.state as staff_state')
                    ->leftJoin('canteen_company_staff_t b', "a.staff_id=b.id")
                    ->leftJoin('canteen_staff_card_t c', "b.id=c.staff_id and c.state<3")
                    ->leftJoin('canteen_company_department_t d', "b.d_id=d.id")
                    ->where('a.company_id', $companyId)
            ...
            ->unionAll(function ($query) use ($companyId, $user, $phone, $department_id) {
                $query->table("canteen_recharge_supplement_t")
                    ->alias('a')
                    ->field('sum(a.money) as money,a.staff_id,b.username,b.code,c.card_code as card_num,b.phone,b.d_id as department_id,d.name as department,b.state as staff_state')
                    ->leftJoin('canteen_company_staff_t b', "a.staff_id=b.id")
                    ->leftJoin('canteen_staff_card_t c', "b.id=c.staff_id and c.state<3")
                    ->leftJoin('canteen_company_department_t d', "b.d_id=d.id")
                    ->where('a.company_id', $companyId)
            ...
            ->unionAll(function ($query) use ($companyId, $user, $phone, $department_id) {
                $query->table("canteen_recharge_cash_t")
                    ->alias('a')
                    ->field('sum(a.money),a.staff_id,b.username,b.code,c.card_code as card_num,b.phone,b.d_id as department_id,d.name as department,b.state as staff_state')
                    ->leftJoin('canteen_company_staff_t b', "a.staff_id=b.id")
                    ->leftJoin('canteen_staff_card_t c', "b.id=c.staff_id and c.state<3")
                    ->leftJoin('canteen_company_department_t d', "b.d_id=d.id")
                    ->where('a.company_id', $companyId)
    ...
    {
        /*        $sql = self::getSqlForStaffsBalance2($department_id, $user, $phone, $company_id);
                if ($checkCard) {
