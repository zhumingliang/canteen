...
class Reception extends BaseController
{
    ...
    public function receptionsForOfficial($page = 1, $size = 5)
    {
        ...
        if ($whereStr !== "") {
            $sql = "select t2.ordering_date,t1.code_number as reception_code,t3.name as canteen_name,t4.name as dinner_name,t2.money,(case when t1.`status` = 1 then '已使用' when t1.`status` = 2 then '未使用' when t1.`status` = 3 then '已取消' when t1.`status` = 4 then '已过期' end) as reception_state,(case when t1.status = 3 then t1.update_time else COALESCE(t1.used_time,'') end) as used_time from canteen_reception_qrcode_t t1 left join canteen_reception_t t2 on t1.re_id=t2.id left join canteen_canteen_t t3 on t2.canteen_id=t3.id left join canteen_dinner_t t4 on t2.dinner_id=t4.id left join canteen_company_staff_t t5 on t2.staff_id=t5.id where 1 = 1 and t5.state=1 and t2.status = 2 " . $whereStr . " order by field(t1.`status`,2,1,3,4),t2.ordering_date desc limit ?,?";
            $count = "select count(*) as count from canteen_reception_qrcode_t t1 left join canteen_reception_t t2 on t1.re_id=t2.id left join canteen_canteen_t t3 on t2.canteen_id=t3.id left join canteen_dinner_t t4 on t2.dinner_id=t4.id left join canteen_company_staff_t t5 on t2.staff_id=t5.id where 1 = 1 and t5.state=1 and t2.status = 2 " . $whereStr;
        } else {
            $sql = "select t2.ordering_date,t1.code_number as reception_code,t3.name as canteen_name,t4.name as dinner_name,t2.money,(case when t1.`status` = 1 then '已使用' when t1.`status` = 2 then '未使用' when t1.`status` = 3 then '已取消' when t1.`status` = 4 then '已过期' end) as reception_state,(case when t1.status = 3 then t1.update_time else COALESCE(t1.used_time,'') end) as used_time from canteen_reception_qrcode_t t1 left join canteen_reception_t t2 on t1.re_id=t2.id left join canteen_canteen_t t3 on t2.canteen_id=t3.id left join canteen_dinner_t t4 on t2.dinner_id=t4.id left join canteen_company_staff_t t5 on t2.staff_id=t5.id where 1 = 1 and t5.state=1 and t2.status = 2 and t2.user_id = " . $user_id . " order by field(t1.`status`,2,1,3,4),t2.ordering_date desc limit ?,?";
            $count = "select count(*) as count from canteen_reception_qrcode_t t1 left join canteen_reception_t t2 on t1.re_id=t2.id left join canteen_canteen_t t3 on t2.canteen_id=t3.id left join canteen_dinner_t t4 on t2.dinner_id=t4.id left join canteen_company_staff_t t5 on t2.staff_id=t5.id where 1 = 1 and t5.state=1 and t2.status = 2 and t2.user_id = " . $user_id;
        }
        $dtResult = Db::query($sql, [($page - 1) * $size, $size]);
        $count = DB::query($count);
        $total = $count[0]['count'];
